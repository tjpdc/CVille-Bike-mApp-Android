...
public class TripData {
	...
	int status;
	float distance;
	String weather, purp, fancystart, info;
	...
	void populateDetails() {
		...
		weather = tripdetails.getString(tripdetails.getColumnIndex("weather"));
		purp = tripdetails.getString(tripdetails.getColumnIndex("purpose"));
	...
	public ItemizedOverlayTrack getPoints(Drawable d) {
		...
		try {
			...
			points.moveToLast();
			this.endpoint = new AccuracyPoint(points.getInt(COL_LAT),
					points.getInt(COL_LGT), points.getDouble(COL_TIME));

			points.moveToFirst();
			this.startpoint = new AccuracyPoint(points.getInt(COL_LAT),
					points.getInt(COL_LGT), points.getDouble(COL_TIME));
	...
	private void addPointToSavedMap(int lat, int lgt, double currentTime,
			float acc) {
		AccuracyPoint pt = new AccuracyPoint(lat, lgt, currentTime, acc);
	...
	boolean addPointNow(Location loc, double currentTime, float dst) {
		...
		AccuracyPoint pt = new AccuracyPoint(lat, lgt, currentTime, accuracy,
				altitude, speed);
	...
	public void updateTrip(String weather, String purpose, String fancyStart,
			String fancyInfo, String notes) {
		// Save the trip details to the phone database. W00t!
		mDb.open();
		mDb.updateTrip(tripid, weather, purpose, startTime, fancyStart,
				fancyInfo, notes, lathigh, latlow, lgthigh, lgtlow, distance);
		mDb.close();
	}
}
