...
public class RecordingService extends Service implements LocationListener {
	...
	public void onLocationChanged(Location loc) {
		...
		// ****************************************************************************************************************************************************
		// When the distance from the starting point of current trip to the end
		// point of last trip is larger than 200 meters. The app indicates that
		// the user forgets to record last trip. So it will request the
		// information of the last unrecorded trip.
		Location cloc = locs.get(0);

		float distance;
		Location lastendpoint = new Location("");
		if (getLastEndPoint() == null)
			distance = 0;
		else {

			lastendpoint
					.setLatitude(((getLastEndPoint().getLatitudeE6()) / 1E6));
			lastendpoint
					.setLongitude(((getLastEndPoint().getLongitudeE6()) / 1E6));
			distance = cloc.distanceTo(lastendpoint);

		}

		if (distance > 200) {

			RecordingService.this.recordActivity.finish();
			locs.clear();

			// startActivity(new Intent(RecordingService.this,
			// MissingTripAlert.class)
			// .setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));

			startActivity(new Intent(RecordingService.this,
					SaveUnrecordedTrip.class)
					.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
		}
		// ****************************************************************************************************************************************************
	...
	public void remindUser() {
		...
		int icon = R.drawable.icon_96x96;
		...
		CharSequence contentTitle = "C'Ville Bike mApp - Recording";
	...
	// *******************************************************************
	// Get the end point of last recorded trip
	public GeoPoint getLastEndPoint() {

		DbAdapter mDb = new DbAdapter(RecordingService.this);

		mDb.open();

		Cursor trips = mDb.fetchAllTrips();
		...
		if (trips != null && trips.getCount() > 0) {
			...
			if (trips.getCount() == 1) {
				...
				int COL_TRIP_ID = trips.getColumnIndex("_id");

				long tripid = trips.getLong(COL_TRIP_ID);

				Cursor points = mDb.fetchAllCoordsForTrip(tripid);

				if (points != null && points.getCount() > 0) {
					points.moveToLast();

					int COL_LAT = points.getColumnIndex("lat");
					int COL_LGT = points.getColumnIndex("lgt");

					int lastlat = (points.getInt(COL_LAT));
					int lastlgt = (points.getInt(COL_LGT));
					...
					return (new GeoPoint(lastlat, lastlgt));
				}
